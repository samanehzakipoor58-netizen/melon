...
export default function ProductsForm({ selcted , onClose }: Props) {
...
const mutate = useMutation({
  mutationFn: (value: Product) => {
    if (selcted) {
      // update
      return axios.patch(
        `http://localhost:3001/api/product/${selcted.id}`,
        value
      );
    } else {
      // create
...
React.useEffect(() => {
  if (selcted) {
    form.setFieldValue("title", selcted.title);
    form.setFieldValue("img", selcted.img);
    form.setFieldValue("price", selcted.price);
  }
}, [selcted]);
  const form = useForm({
